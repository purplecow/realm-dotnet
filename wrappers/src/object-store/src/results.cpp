...
//ASD                                         Double agg_double, Timestamp agg_timestamp)
{
    validate_read();
    if (!m_table)
        return none;
    ...
    {
//ASD        case type_Timestamp: return do_agg(agg_timestamp);
...
//ASD                     [=](auto const& table) { return table.maximum_timestamp(column); });
}
...
//ASD                     [=](auto const& table) { return table.minimum_timestamp(column); });
}
